...
//			}
		...
		public int count() {
			...
			log.debug(buildLog("准备计算记录条数：" + sql + "\t" + Arrays.toString(params)));
			int result = jdbcTemplate.queryForObject(sql, params, Integer.class);
			log.debug(buildLog("计算记录条数为：" + result));
			return result;
		}
		...
		public boolean hasRecord() {
			...
			log.debug(buildLog("是否有纪录：" + result));
			return result;
		}
	...
	public <T> Optional<T> selectByPrimaryKey(Object id) {
		try {
			...
			log.debug(buildLog("根据ID查找到：" + t));
			return Optional.of(t);
		} catch (EmptyResultDataAccessException e) {
			log.debug(buildLog("根据ID(" + id + ")查找不到对象"));
	...
	public long addAndReturnKey(Object object) {
		...
		log.debug(buildLog("插入成功:" + object + "\t" + "primary key为:" + id));
		return id;
	}
	...
	public <T> Optional<T> execSelectSqlAsPojoOpt(String sql, Object... args) {
		try {
			...
			log.debug(buildLog("查找到记录：" + t));
			return Optional.of(t);
		} catch (EmptyResultDataAccessException e) {
			log.debug(buildLog("没有查找到数据"));
			return Optional.empty();
		}
	}
	...
	public int execUpdate(String sql, Object... args) {
		log.debug(buildLog("执行修改操作：" + sql + "\t" + Arrays.toString(args)));
		int result = jdbcTemplate.update(sql, args);
		log.debug(buildLog("执行修改操作条数：" + result));
		return result;
	}
}